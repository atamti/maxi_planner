...
export const DraggableRateChart: React.FC<Props> = ({
  ...
  const height = 300;
  const paddingLeft = 90; // Increased left padding for axis labels
  ...
  const handleMouseDown = (event: React.MouseEvent<SVGSVGElement>) => {
    if (readOnly) return; // Don't allow interaction if readOnly
  ...
  const handleMouseMove = useCallback(
    ...
    [isDragging, dragIndex, data, onChange, height, readOnly],
  );
  ...
  // Add these new handlers for document events
  ...
  // Add global mouse event listeners with the correct handler functions
  React.useEffect(() => {
    if (isDragging) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <text
          x={35} // Change from 15 to 5 to move it further left
          y={height / 2}
          textAnchor="middle"
          fontSize="14"
          fill="#333"
          fontWeight="bold"
          transform={`rotate(-90, 30, ${height / 2})`} // Also update the rotation anchor point
        >
